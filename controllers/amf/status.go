...
func createNfDeploymentStatus(deployment *appsv1.Deployment, amfDeployment *nephiov1alpha1.AMFDeployment) (nephiov1alpha1.NFDeploymentStatus, bool) {
	...
	if len(amfDeployment.Status.Conditions) == 0 {
		nfDeploymentStatus.Conditions = append(nfDeploymentStatus.Conditions, metav1.Condition{
			...
			Message:            "AMFDeployment pod(s) is(are) starting.",
			LastTransitionTime: metav1.Now(),
		})

		return nfDeploymentStatus, true
	} else if (len(deployment.Status.Conditions) == 0) && (len(amfDeployment.Status.Conditions) > 0) {
	...
	lastAmfDeploymentCondition := amfDeployment.Status.Conditions[len(amfDeployment.Status.Conditions)-1]
	...
	case appsv1.DeploymentAvailable:
		nfDeploymentStatus.Conditions = append(nfDeploymentStatus.Conditions, metav1.Condition{
			...
			Message:            "AMFDeployment pods are available.",
			LastTransitionTime: metav1.Now(),
		})
	...
	case appsv1.DeploymentReplicaFailure:
		nfDeploymentStatus.Conditions = append(nfDeploymentStatus.Conditions, metav1.Condition{
			...
			Status:             metav1.ConditionFalse,
			Reason:             "MinimumReplicasNotAvailable",
			Message:            "AMFDeployment pod(s) is(are) failing.",
			LastTransitionTime: metav1.Now(),
		})
	}
