...
func getSMFResourceParams(capacity workloadv1alpha1.SMFCapacity) (int32, *apiv1.ResourceRequirements, error) {
...
// getNads returns NAD label string composed based on the Nx interfaces configuration provided in UPFDeploymentSpec
...
func (r *SMFDeploymentReconciler) checkNADexist(log logr.Logger, ctx context.Context, deployment *appsv1.Deployment) bool {
	...
	for _, annotation := range annotations {
		...
		if err := r.Get(ctx, key, u); err != nil {
			log.Info(fmt.Sprintf("Failed to get NAD %s", annotation.Name), "SMFDeployment.namespace", deployment.Namespace)
			return false
		}
	}
...
func free5gcSMFDeployment(log logr.Logger, smfDeploy *workloadv1alpha1.SMFDeployment) (*appsv1.Deployment, error) {
	...
	replicas, resourceReq, err := getSMFResourceParams(spec.Capacity)
	if err != nil {
		return nil, err
	}
	instanceNadLabel := getSMFNad(smfDeploy.ObjectMeta.Name, &spec)
...
func free5gcSMFCreateConfigmap(logger logr.Logger, smfDeploy *workloadv1alpha1.SMFDeployment) (*apiv1.ConfigMap, error) {
	namespace := smfDeploy.ObjectMeta.Namespace
	instanceName := smfDeploy.ObjectMeta.Name
	n4IP, _, _ := net.ParseCIDR(smfDeploy.Spec.N4Interfaces[0].IPs[0])
	n3IP, _, _ := net.ParseCIDR(smfDeploy.Spec.N3Interfaces[0].IPs[0])

	smfcfgStruct := UPFcfgStruct{}
	smfcfgStruct.PFCP_IP = n4IP.String()
	smfcfgStruct.GTPU_IP = n3IP.String()
	// smfcfgStruct.N6cfg = smfDeploy.Spec.N6Interfaces
...
func (r *SMFDeploymentReconciler) syncStatus(ctx context.Context, d *appsv1.Deployment, smfDeploy *workloadv1alpha1.SMFDeployment) error {
	newStatus, update := calculateSMFStatus(d, smfDeploy)

	if update {
		// Update UPFDeployment status according to underlying deployment status
		newSmf := smfDeploy
		newSmf.Status = newStatus
...
func calculateSMFStatus(deployment *appsv1.Deployment, smfDeploy *workloadv1alpha1.SMFDeployment) (workloadv1alpha1.SMFDeploymentStatus, bool) {
	status := workloadv1alpha1.SMFDeploymentStatus{
	...
	if lastDeploymentStatus.Type == appsv1.DeploymentProgressing && lastSMFDeploymentStatus.Type == workloadv1alpha1.Reconciling {
		return status, false
	}
...
func (r *SMFDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if dmFound {
		...
		if d.DeletionTimestamp == nil {
			if err := r.syncStatus(ctx, d, smfDeploy); err != nil {
	...
	SMFDeploymentFinalizer := "smfdeployment.5gcore.workload.nephio.org/finalizer"
	if smfDeploy.ObjectMeta.DeletionTimestamp.IsZero() {
		if !controllerutil.ContainsFinalizer(smfDeploy, SMFDeploymentFinalizer) {
			controllerutil.AddFinalizer(smfDeploy, SMFDeploymentFinalizer)
			if err := r.Client.Update(ctx, smfDeploy); err != nil {
				return reconcile.Result{}, err
			}
		}
	} else {
		if controllerutil.ContainsFinalizer(smfDeploy, SMFDeploymentFinalizer) {
			log.Info(fmt.Sprintf("Deleting SMF ConfigMap and Deployment: %v\n", smfDeploy.ObjectMeta.Name))
			if cmFound {
			...
			controllerutil.RemoveFinalizer(smfDeploy, SMFDeploymentFinalizer)
			if err := r.Client.Update(ctx, smfDeploy); err != nil {
				return reconcile.Result{}, err
			}
		}
		...
		if !dmFound {
			...
			if ok := r.checkNADexist(log, ctx, deployment); !ok {
				...
				if err := ctrl.SetControllerReference(smfDeploy, deployment, r.Scheme); err != nil {
					log.Error(err, "Got error while setting Owner reference.", "SMFDeployment.namespace", namespace)
				}
